...
%type <i_val> WhileStmt

%type IfStmt IfOnlyStmt IfElseStmt
...
IfElseStmt
    : IF Condition IfElseLabels
    {
        printf("[parser] matched IfElseStmt (with else)\n");
        int else_lbl = ($3 >> 16) & 0xFFFF;
        CODEGEN("iload %d", $2);
        CODEGEN("ifeq Else_%d", else_lbl);
    }
    StmtBlock OptNewline
    {
        int else_lbl = ($3 >> 16) & 0xFFFF;
        int exit_lbl = ($3      ) & 0xFFFF;
...
Statement
    : IfStmt
    | IfStmt OptNewline ELSE StmtBlock
    | WhileStmt
    | LET ID '=' Expression ';'
        {
            int loc = $4.local_idx;
